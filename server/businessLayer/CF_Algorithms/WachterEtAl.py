...
def initAlgo(model,arg_lst,data=None):
    MADs = arg_lst["MADs"]
    f = model.predict
    learning_rate = arg_lst["learning_rate"]
    ytag = arg_lst["ytag"]
    epsilon = arg_lst["epsilon"]
    Lambda = arg_lst["Lambda"]
    ranges = arg_lst["ranges"]
    max_iter = arg_lst["max_iter"] if "max_iter" in arg_lst else 300
